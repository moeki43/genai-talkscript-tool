...
with st.expander("📃&💬"):
    width_left = st.slider('Layout setting',  min_value=0, max_value=10, value=7)
    ax_left,ax_right = st.columns([1+width_left,1+10-width_left])
container_graph = st.container(height=300)
container_graph.write("🧭Talk script chart")


container_table = ax_left.container(height=450)
container_chat = ax_right.container(height=450)
...
with container_table:
    ...
    btn_create = container_table_cols[0].button('Create questions table')
    btn_questions_order = container_table_cols[1].button('Rearrange questions')
    btn_qustions_links = container_table_cols[2].button('Generate links')
